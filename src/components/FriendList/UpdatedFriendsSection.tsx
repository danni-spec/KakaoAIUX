...
const UPDATED_FRIENDS = [
  { name: "카나나저널", hasUpdate: true, isAd: false, photo: "/voice-effect.png" },
      ...
      <div className="flex gap-3 overflow-x-auto px-4 pt-2 pb-1 scrollbar-hide">
        {UPDATED_FRIENDS.map((friend) => (
          <div
            ...
            style={friend.name === "카나나저널" ? { touchAction: "manipulation" } : undefined}
            onClick={(e) => {
              e.stopPropagation();
            ...
            <div className="relative" style={{ perspective: "200px" }}>
              {friend.photo ? (
                <SquircleAvatar
                  src={friend.photo}
                  alt={friend.name}
                  className={`w-[48px] h-[48px] ${friend.name === "카나나저널" ? "animate-flip-y" : ""}`}
                  noOutline={friend.name === "카나나저널"}
                />
              ) : (
